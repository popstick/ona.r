...
In this demo, we will use the `ona` and `tm` (text mining) packages in R to download data from ona, and look at answers from a survey on schoools. Before we go any further, we will load our schools dataset into R. If you haven't used ona.R before, please read the [basics tutorial](http://sel-columbia.github.io/ona.R/demo/Basics_of_ona.R.html) to learn how to load your own ona data into R.

```{r message=FALSE, cache=TRUE}
require(ona) # See http://sel-columbia.github.io/ona.R/ for install instructions if you get an error
require(tm)      # Run install.packages('tm') if you get an error
edu <- onaRead("~/Downloads/mopup_questionnaire_education_final_2014_04_02_08_15_46.csv",
...
---
We hope that you are able to benefit from similar analysis yourself as well. If you have R installed, you should be able to run R code very similar to what you see on this page, as long as you put in the correct filenames and column names. In fact, the last block of code is designed to be copy-pasted; as long as you change the line marked CHANGE, you should be able to reproduce this analysis for your own datase. For questions, please contact the ona-users [Google group](https://groups.google.com/d/forum/ona-users).
