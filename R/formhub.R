...
formhubDownload = function(formName, uname, pass=NA, authfile=NA, url='https://classic.ona.io/', ...) {
  ...
  htmlFromCol <- function(photoCol, size, type) {
    ...
    if (type == "url") { 
      ifelse(is.na(photoCol), "",
           sprintf("https://classic.ona.io/attachment/%s?media_file=%s/attachments/%s",
                   size, formhubUsername, photoCol))
    } else if (type == "img") {
      ifelse(is.na(photoCol), "",
             sprintf('<img src="https://classic.ona.io/attachment/%s?media_file=%s/attachments/%s" />',
                     size, formhubUsername, photoCol))
    } else { 
