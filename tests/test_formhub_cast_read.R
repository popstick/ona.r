...
form_to_df = ona:::form_to_df
recastDataFrameBasedOnFormDF = ona:::recastDataFrameBasedOnFormDF
...
test_that("@form has nice row.names", {
    ...
    expect_warning(edu_onaObj <- onaRead(edu_datafile, edu_formfile))
    expect_equal(row.names(edu_onaObj@form), edu_onaObj@form$name)
